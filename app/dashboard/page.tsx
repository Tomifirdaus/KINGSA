...
export default function DashboardPage() {
  ...
  // Menambahkan properti 'path' untuk rute tujuan navigasi
  const sidebarMenus = [
    { name: 'Dashboard', icon: LayoutDashboard, path: '/dashboard' },
    { name: 'Member', icon: Users, path: '/member' },       
    { name: 'Pembeli', icon: ShoppingBag, path: '/pembeli' }, 
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 flex font-sans antialiased overflow-x-hidden">
            ...
            {sidebarMenus.map((menu, idx) => {
              ...
              return (
                <Link
                  ...
                  // Menghapus total warna kuning, diganti abu-abu terang dengan efek hover yang lembut
                  className="flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-semibold text-slate-300 hover:bg-slate-800/40 hover:text-white transition-all"
                >
      ...
      <div className="fixed bottom-4 right-4 md:bottom-5 md:right-5 z-40">
        <a 
          href="https://wa.me/6285198185451?text=Halo%20Admin,%20saya%20tertarik%20dengan%20produk%20Anda."
          target="_blank" 
          rel="noopener noreferrer"
          className="w-10 h-10 md:w-11 md:h-11 bg-[#2ECC71] text-white rounded-full flex items-center justify-center shadow-lg hover:bg-[#27AE60] transition-transform hover:scale-110"
        >
